...
class BM25Index:
    ...
    doc_freqs: Dict[str, int] = field(default_factory=dict)  # Term -> number of docs containing term
    term_freqs: List[Dict[str, int]] = field(default_factory=list)  # Per-doc term frequencies
    num_docs: int = 0
    ...
    def save(self, path: str) -> None:
        ...
        logger.info(f"Saved BM25 index to {path} with {self.num_docs} documents")
    ...
    def load(cls, path: str) -> "BM25Index":
        ...
        logger.info(f"Loaded BM25 index from {path} with {index.num_docs} documents")
        return index
...
def get_bm25_index_path(collection_name: str = "grant_library") -> str:
    ...
    return os.path.join(backend_app_dir, "data", "app_library", "vector_store", f"{collection_name}_bm25.pkl")


def get_bm25_index(collection_name: str = "grant_library") -> BM25Index:
    """Get or load a BM25 index."""
